...
from utils.io_utils import create_zip_file, load_image_pil, save_df_result, save_uploaded_file, IMAGE_DIR, RESULT_DIR
...
if uploaded_files == []:
    ...
    if uploaded_files != []:
        ...
        file_path = IMAGE_DIR / picture_name
    ...
    if st.button("Launch Rugosity Calculation"):
        ...
        for idx, file_ in enumerate(uploaded_files):
            ...
            with st.spinner(f"""{picture_name} ({idx}/{len(uploaded_files)})"""):
                ...
                plt.savefig(RESULT_DIR / f"{picture_name}_result.png")
